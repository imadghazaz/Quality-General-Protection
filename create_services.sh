#!/bin/bash

# Service data: filename|title|image|description
services=(
    "detection-incendie.html|Détection Incendie|Detection-Incendie.jpg|Systèmes de détection d'incendie et asservissements pour une sécurité maximale"
    "stations-pompage.html|Stations de Pompage|Stations-de-Pompage.png|Installation et maintenance de stations de pompage pour systèmes anti-incendie"
    "portes-coupe-feu.html|Portes Coupe-Feu|Portes-Coupe-Feu.png|Installation et maintenance de portes coupe-feu et portes de secours"
    "desenfumage.html|Désenfumage|Desenfumage.png|Installation d'exutoires de désenfumage pour l'évacuation des fumées"
    "signaletique-evacuation.html|Signalétique d'Évacuation|Signaletique-Evacuation.png|Plans, traçage et plaques d'évacuation conformes aux normes"
    "extinction-automatique.html|Extinction Automatique|Extinction-Automatique.png|Systèmes d'extinction automatique pour une réponse immédiate aux incendies"
)

for service in "${services[@]}"; do
    IFS='|' read -r filename title image description <<< "$service"
    
    cat > "$filename" << EOF
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>$title - QGP Fire Protection</title>
    <link rel="stylesheet" href="assets/css/icons.min.css">
    <link rel="stylesheet" href="assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="assets/css/responsive.css">
    <link rel="shortcut icon" type="image/png" href="assets/images/favicon.png"/>
    <link rel="stylesheet" href="assets/css/colors/color.css" title="color" />
</head>
<body itemscope>
    <div class="preloader"><div class="loader-inner ball-scale-multiple"><div></div><div></div><div></div></div></div>
    <main>
        <header class="stick">
            <div class="tb-br">
                <div class="container">
                    <div class="scl1 float-left">
                        <a href="#" title="Twitter" itemprop="url" target="_blank"><i class="fab fa-twitter"></i></a>
                        <a href="#" title="Facebook" itemprop="url" target="_blank"><i class="fab fa-facebook-f"></i></a>
                        <a href="#" title="Linkedin" itemprop="url" target="_blank"><i class="fab fa-linkedin-in"></i></a>
                        <a href="#" title="Google Plus" itemprop="url" target="_blank"><i class="fab fa-google-plus-g"></i></a>
                        <a href="#" title="Instagram" itemprop="url" target="_blank"><i class="fab fa-instagram"></i></a>
                    </div>
                    <ul class="tp-lst float-right">
                        <li><i class="fas fa-envelope theme-clr"></i><a href="#" title="" itemprop="url">info@example.com</a></li>
                        <li><i class="flaticon-telephone theme-clr"></i>+(00) 123-345-678</li>
                    </ul>
                </div>
            </div>
            <div class="lg-mnu-sec">
                <div class="container">
                    <div class="logo"><a href="index.html" title="Logo" itemprop="url"><img src="assets/images/logo.png" alt="logo.png" itemprop="image"></a></div>
                    <nav>
                        <div>
                            <ul>
                                <li class = "menu-item-has-children"><a class="brd-rd3" href="index.html" title="" itemprop="url">Home <i class="fas fa-angle-down"></i></a>
									<ul>
                                        <li><a href="index.html" title="" itemprop="url">Home 1</a></li>
                                        <li><a href="index-2.html" title="" itemprop="url">Home 2</a></li>
                                    </ul>
								</li>
								<li><a href="about.html" title="" itemprop="url">About Us</a></li>
								<li><a href="campaign.html" title="" itemprop="url">Campaigns</a></li>
                                <li class="menu-item-has-children"><a href="#" title="" itemprop="url">Events <i class="fas fa-angle-down"></i></a>
                                    <ul>
                                        <li><a href="events.html" title="" itemprop="url">Our Events</a></li>
                                        <li><a href="event-detail.html" title="" itemprop="url">Event Detail</a></li>
                                    </ul>
                                </li>
								<li class="menu-item-has-children"><a href="#" title="" itemprop="url">Our Blog <i class="fas fa-angle-down"></i></a>
                                    <ul>
                                        <li><a href="blog.html" title="" itemprop="url">Blog Grid</a></li>
                                        <li><a href="blog-detail.html" title="" itemprop="url">Blog Detail</a></li>
                                    </ul>
                                </li>
								<li class="menu-item-has-children"><a href="#" title="" itemprop="url">Pages <i class="fas fa-angle-down"></i></a>
                                    <ul>
                                        <li><a href="service.html" title="" itemprop="url">Offered Services</a></li>
                                        <li><a href="operation-videos.html" title="" itemprop="url">Operation Videos</a></li>
										<li><a href="team.html" title="" itemprop="url">Fire Fighters</a></li>
										<li><a href="team-2.html" title="" itemprop="url">Fire Fighters 2</a></li>
                                    </ul>
                                </li>
                                <li><a href="contact.html" title="" itemprop="url">Contact</a></li>
                            </ul>
                            <a class="srch-btn theme-bg brd-rd3" href="#" title="" itemprop="url"><i class="fas fa-search"></i></a>
                        </div>
                    </nav>
                </div>
            </div>
        </header>
        <section>
            <div class="gap black-layer opc8 overlap144">
                <div class="fixed-bg2" style="background-image: url(assets/images/pg-tp-bg.jpg);"></div>
                <div class="container">
                    <div class="pg-tp-wrp">
                        <h1 itemprop="headline">$title</h1>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.html" title="" itemprop="url">Home</a></li>
                            <li class="breadcrumb-item"><a href="index.html#services" title="" itemprop="url">Services</a></li>
                            <li class="breadcrumb-item active">$title</li>
                        </ol>
                    </div>
                </div>
            </div>
        </section>
        <section>
            <div class="gap">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-lg-12">
                            <div class="service-detail-content">
                                <div class="text-center" style="margin-bottom: 40px;">
                                    <img src="assets/images/$image" alt="$title" style="width: 200px; height: 200px; object-fit: contain; border-radius: 50%;">
                                </div>

                                <h2 itemprop="headline">$title</h2>
                                <p itemprop="description">$description</p>

                                <h3>Nos Services</h3>
                                <p>QGP offre des services complets pour $title avec une expertise reconnue dans le domaine de la protection incendie.</p>
                                <ul style="list-style-type: disc; margin-left: 30px; line-height: 2;">
                                    <li><strong>Installation professionnelle</strong> - Mise en place conforme aux normes</li>
                                    <li><strong>Maintenance préventive</strong> - Contrôles réguliers et entretien</li>
                                    <li><strong>Vérification périodique</strong> - Inspection complète et certification</li>
                                    <li><strong>Service après-vente</strong> - Support technique et dépannage</li>
                                    <li><strong>Conseil et expertise</strong> - Accompagnement personnalisé</li>
                                </ul>

                                <h3>Avantages</h3>
                                <ul style="list-style-type: disc; margin-left: 30px; line-height: 2;">
                                    <li>Équipements certifiés et conformes aux normes</li>
                                    <li>Techniciens qualifiés et expérimentés</li>
                                    <li>Intervention rapide sur toute la région</li>
                                    <li>Service client disponible et réactif</li>
                                    <li>Garantie de qualité et de conformité</li>
                                </ul>

                                <h3>Conformité et Sécurité</h3>
                                <p>Tous nos services respectent les réglementations en vigueur et les normes de sécurité incendie. Nous garantissons une conformité totale aux exigences légales.</p>

                                <div style="margin-top: 40px; padding: 30px; background-color: #f8f9fa; border-radius: 5px;">
                                    <h3>Besoin d'informations ?</h3>
                                    <p>Contactez-nous pour discuter de vos besoins en matière de $title.</p>
                                    <a href="contact.html" class="theme-btn brd-rd3" style="display: inline-block; margin-top: 15px;">Contactez-nous</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <footer>
            <div class="gap drk-bg">
                <div class="container">
                    <div class="ftr-dta remove-ext5">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="wdgt-bx text-center">
                                    <p style="color: #fff;">&copy; 2024 QGP Fire Protection. All Rights Reserved.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    </main>
    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/js/bootstrap.min.js"></script>
    <script src="assets/js/custom-scripts.js"></script>
</body>
</html>
EOF
    
    echo "Created $filename"
done

echo "All service pages created successfully!"
